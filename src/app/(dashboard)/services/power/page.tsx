...
export default function ElectricityPage() {
  ...
  useEffect(() => {
    if (!disco || meterNumber.trim().length < 8) {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          setCustomerName(null);
          setCustomerAddress(null);
          setValidationError(data.error || 'Meter verification failed');
        }
      } catch (err: any) {
        setValidationError('Failed to reach validation service');
      } finally {
        setValidating(false);
      }
    }, 600);
  ...
  const handlePurchase = async (e: React.FormEvent) => {
    ...
    if (!amount || numAmount < 500) {
      error('Enter Payment Amount', 'Minimum electricity payment is ₦500');
      return;
    }
    ...
    if (currentBalance < numAmount) {
      error(
        ...
        `You need ${formatNaira(numAmount)}, but your balance is ${formatNaira(currentBalance)}. Please fund your wallet.`,
        {
          label: 'Fund Wallet',
    ...
    <div className="max-w-xl mx-auto space-y-6 animate-in fade-in">
      ...
      <div className="rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900/80 p-6 md:p-8 backdrop-blur-xl shadow-xl space-y-6">
        <form onSubmit={handlePurchase} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                required
                className="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-white/10 text-slate-900 dark:text-white font-mono text-sm focus:outline-none focus:border-emerald-500 pr-24"
              />
              {validating && (
            ...
            {customerName && (
              <div className="mt-2 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-start gap-2.5 text-xs text-emerald-600 dark:text-emerald-300">
          ...
          <button
            type="submit"
            disabled={loading || validating || !disco || !meterNumber || numAmount < 500}
