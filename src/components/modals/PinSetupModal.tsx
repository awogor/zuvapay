...
export function PinSetupModal({ isOpen, onSuccess }: PinSetupModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-xl animate-in fade-in duration-300">
      <div className="relative w-full max-w-md rounded-3xl border border-white/10 bg-slate-900/95 p-6 sm:p-8 shadow-2xl backdrop-blur-2xl text-center overflow-hidden">
        ...
        <div className="flex items-center justify-center gap-2 my-5">
          ...
          <span
            className={`h-1.5 rounded-full transition-all duration-300 ${
              step === 'confirm' ? 'w-8 bg-brand-orange' : 'w-4 bg-slate-700'
            }`}
          />
        </div>
            ...
            <div className="flex items-center justify-center gap-3 sm:gap-4 my-6">
              {pinDigits.map((digit, idx) => (
                <input
                  ...
                  className="w-12 h-14 sm:w-14 sm:h-16 text-center text-2xl font-black rounded-2xl bg-slate-950/80 border border-white/15 text-white focus:outline-none focus:border-brand-orange focus:ring-2 focus:ring-brand-orange/20 transition-all shadow-inner"
                />
              ))}
            </div>

            {pinError && (
              <div className="p-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-400 text-xs flex items-center justify-center gap-2">
        ...
        {/* Security badge note */}
        <div className="mt-6 pt-4 border-t border-white/5 flex items-center justify-center gap-2 text-[11px] text-slate-400 font-medium">
