...
interface FaddedPricingModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function FaddedPricingModal({ isOpen, onClose }: FaddedPricingModalProps) {
  ...
  // Inline editing state: mapping product_key -> edited price string
  const [editingOverrides, setEditingOverrides] = useState<Record<string, string>>({});
  const [updatingId, setUpdatingId] = useState<string | null>(null);

  const fetchFaddedPricing = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/admin/pricing?provider=fadded');
      const data = await res.json();
      if (data.success) {
        setPricingConfig(data.config);
        setProducts(data.fadded?.products || []);

        const activeRule = data.config?.fadded?.globalRule;
        if (activeRule) {
        ...
        error('Failed to load', data.error || 'Could not fetch Fadded pricing');
      }
    } catch (err: any) {
  ...
  const handleSaveGlobalRule = async () => {
    ...
    try {
      ...
      if (resData.success) {
        success('Margin Updated', 'Global Fadded retail margin rule saved successfully!');
        fetchFaddedPricing();
      } else {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-950/70 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-3xl w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-black text-slate-900 dark:text-white">
                  Fadded Reseller v2 Products & Pricing Manager
                </h3>
        ...
        <div className="p-4 mx-6 my-4 rounded-2xl bg-indigo-50 dark:bg-indigo-500/5 border border-indigo-200/70 dark:border-indigo-500/20 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-slate-900 dark:text-white">
                Global Margin Rule for Fadded Products
              </p>
              <p className="text-[11px] text-slate-500 dark:text-slate-400">
                Applied to Fadded wholesale cost across all accounts, VPNs, RDPs, and proxies unless overridden.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <input
                ...
                className="w-24 px-3 py-1.5 rounded-xl bg-white dark:bg-slate-950 border border-slate-300 dark:border-white/10 text-xs font-bold text-slate-900 dark:text-white text-right focus:outline-none focus:border-indigo-500"
            ...
            <button
              ...
              disabled={saving}
              className="px-4 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs flex items-center gap-1.5 shadow-sm disabled:opacity-50 transition-all"
            >
        ...
        <div className="px-6 py-2 flex flex-wrap items-center justify-between gap-3">
          <div className="relative flex-1 min-w-[240px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 text-xs text-slate-900 dark:text-white font-medium focus:outline-none focus:border-indigo-500"
            />
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-2">
          {loading ? (
            <div className="py-20 text-center space-y-3">
              ...
              <p className="text-xs text-slate-500 font-medium">Loading Fadded products & active pricing...</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-900/60 border-b border-slate-200 dark:border-white/10 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                    ...
                    <th className="py-3 px-4">Fadded Wholesale</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-white/5 font-medium">
                  {filteredProducts.map((p) => {
                    const isEditing = editingOverrides[p.product_key] !== undefined;
                    const isUpdating = updatingId === p.product_key;
                    return (
                      <tr key={p.product_key} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors">
                        <td className="py-3 px-4">
                          <div className="font-bold text-slate-900 dark:text-white line-clamp-1">{p.name}</div>
                          <div className="text-[11px] text-slate-400 font-mono">
                            Category: {p.category} · Key: {p.product_key}
                          </div>
                        </td>
                          ...
                          <div className="flex items-center justify-end gap-1.5">
                            {isEditing ? (
                              <>
                                <input
                                  type="number"
                                  value={editingOverrides[p.product_key]}
                                  onChange={(e) =>
                                    setEditingOverrides({
                                      ...editingOverrides,
                                  ...
                                  placeholder="Selling ₦"
                                  className="w-24 px-2 py-1 rounded-lg bg-white dark:bg-slate-900 border border-indigo-500 text-slate-900 dark:text-white font-mono text-xs text-right focus:outline-none"
                                  autoFocus
                                />
                                <button
                                  onClick={() => handleSaveOverride(p.product_key)}
                                  disabled={isUpdating}
                              ...
                              <>
                                <button
                                  onClick={() =>
                                   setEditingOverrides({
                                      ...editingOverrides,
                                      [p.product_key]: String(p.retailPrice),
                                    })
                                  }
                                ...
                                {p.isOverridden && (
                                  <button
                                    onClick={() => handleClearOverride(p.product_key)}
                                    disabled={isUpdating}
        ...
        <div className="p-4 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-slate-950/40 flex items-center justify-between text-xs text-slate-500">
          <span>
            Showing {filteredProducts.length} Fadded products ({categories.length - 1} categories)
          </span>
          <button
            onClick={onClose}
